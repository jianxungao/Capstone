...
public class TestProvider extends AndroidTestCase {
    ...
    public void testBasicCountryQuery() {
        ...
        assertTrue("Unable to Insert data to db ", rowId != -1);
        db.close();
...
//            countryValues.put(CountryContract.CountryEntry.NAME, new String(TEST_COUNTRY_NAME+i));
    ...
    public void testBulkInsert(){
        ...
        ContentValues[] bulkInsertCV = createBulkInsertCountryVal();

        TestUtilities.TestContentObserver tco = TestUtilities.getTestContentObserver();
        mContext.getContentResolver().registerContentObserver(CountryContract.CountryEntry.CONTENT_URI,true,tco);

        int insertCount = mContext.getContentResolver().bulkInsert(CountryContract.CountryEntry.CONTENT_URI, bulkInsertCV);
        ...
        Cursor c = mContext.getContentResolver().query(
                ...
                null,
                null,
                CountryContract.CountryEntry.NAME + " ASC"
        );
        ...
        for ( int i = 0; i < BULK_INSERT_RECORDS_TO_INSERT; i++, c.moveToNext() ){
            TestUtilities.validateCurrentRecord("testBulkInsert. Error validating CountryEntry " + i,
             c, bulkInsertCV[i]);
        }
